...
for i in range(len(raw)):
  ...
  if all(check):
    values.append(n)
    erooms[n] = (rs.group(1)).replace('-',' ')
#print(erooms)

for m,n in erooms.items():
  scratchpaper = []
  for d in n: 
    if d == ' ':
      scratchpaper.append(d)
    else:
      scratchpaper.append(alph[(alph.index(d)+m)%26])
  rrooms[m] = ''.join(scratchpaper)
