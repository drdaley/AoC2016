...
def doorcheck(pos): #from pos, returns a list, to be zipped with c, of T/F if a door exists/doesn't
  ...
  elif pos[1] == -4: doors.extend([True,False])
  elif 0 > pos[1] > -4: doors.extend([True,True])
  if pos[0] == 0: doors.extend([False,True])
  elif pos[0] == 4: doors.extend([True,False])
  elif 0 > pos[0] > 4: doors.extend([True,True])
  return doors
  
def endcheck(gp):
  for g in gp:
    if findpos(g) == (4,-4): return True
  return False
